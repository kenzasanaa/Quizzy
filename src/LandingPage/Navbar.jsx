      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-20 flex items-center justify-between">
        ...
        <div className="hidden md:flex items-center gap-4">
          <button 
            ...
            className="text-[#FF7AB6] border border-[#FF7AB6]/30 hover:border-[#FF7AB6] font-semibold px-6 py-2.5 rounded-full text-sm transition-colors bg-[#FF7AB6]/5"
          >
            Sign In
          </button>
          <button 
            onClick={() => navigateTo('register')}
            className="bg-[#FFD166] hover:bg-[#FFB86B] text-[#1B1026] font-bold px-6 py-2.5 rounded-full text-sm transition-colors shadow-md shadow-[#FFD166]/10"
          >
            Register
          </button>
        </div>
      ...
      {mobileMenuOpen && (
        <div className="md:hidden bg-[#1B1026] border-b border-[#FF7AB6]/10 px-4 py-6 space-y-4 absolute w-full left-0 transition-all duration-300 shadow-xl">
          ...
          <div className="flex flex-col gap-3 pt-4 border-t border-[#FF7AB6]/10">
            <button 
              onClick={() => { setMobileMenuOpen(false); navigateTo('signin'); }}
